...
(function(){
  ...
  function renderVirtual(f){
    ...
    const gpuSummary=gpu&&gpu.ok?`<div class="gpuBox"><h4><span>⚡ GPU Vision V31.7.1 · pipeline PRO V32</span><small>${esc(gpu.version||'31.7')}</small></h4><div class="gpuFacts"><span>${gpu.images?.renderPro2D?'render 2D pronto':'render da creare'}</span><span>${labelSrc?'etichetta pronta':'etichetta da estrarre'}</span><span>${model.front?'3D V32 pronto':'3D da creare'}</span><span>profilo: solo manuale</span></div></div>`:`<div class="gpuBox"><h4><span>⚡ GPU Vision V31.7.1</span><small>pronta</small></h4><div class="gpuFacts"><span class="warn">Usa i tre pulsanti: Render PRO, Etichetta, Render 360° 3D. Ogni funzione salva in memoria senza cambiare profilo da sola.</span></div></div>`;
    return `<div class="section"><h3>🧊 Render AI · GPU Vision V31.7.1</h3>
      ...
      <div class="v30Info">V31.7.1: collegata al worker RunPod V32: Render PRO usa /render-pro, Etichetta usa /label-pro, Render 360° 3D usa /render-3d con fronte + retro e frames orbit generati dalla GPU.</div>
      ${gpuSummary}
      <div class="renderPipeline">
        ...
        <div class="v30Card"><h4><span>Render 360° 3D</span><small>${esc(model.mode||'v32_depth_orbit')}</small></h4>${frames.length?`<div class="pro3dStage"><img class="pro3dFrameImg" id="gpu3dFrame" src="${esc(frames[0])}" alt="Frame 3D V32"></div>`:`<div class="pro3dStage"><div class="pro3dModel" id="gpu3dModel"><div class="pro3dShadow"></div><div class="pro3dSide pro3dLeft"></div><div class="pro3dSide pro3dRight"></div><div class="pro3dPlane pro3dFront"><img src="${esc(front||renderSrc||fallback)}" alt="Fronte 3D"></div><div class="pro3dPlane pro3dBack"><img src="${esc(back||front||renderSrc||fallback)}" alt="Retro 3D"></div></div></div>`}<textarea id="gpu3dFramesJson" hidden readonly>${esc(JSON.stringify(frames))}</textarea><div class="pro3dControls"><input id="gpu3dRange" type="range" min="0" max="${frames.length?frames.length-1:360}" value="0"><div class="pro3dBtns"><button class="primary mini" data-3d-deg="0">Fronte</button><button class="ghost mini" data-3d-deg="${frames.length?Math.round((frames.length-1)/4):90}">Lato</button><button class="ghost mini" data-3d-deg="${frames.length?Math.round((frames.length-1)/2):180}">Retro</button></div><div class="gpu360Hint" id="gpu3dLabel">${frames.length?`Frame GPU V32: 1/${frames.length}`:`Ruota 3D: 0°. ${back?'Retro caricato in memoria.':'Se manca retro, uso la vista frontale finché non carichi/assegni retro.'}`}</div></div><div class="v30Facts"><span class="${frames.length?'ok':(back?'ok':'warn')}">${frames.length?`${frames.length} frame V32`:back?'fronte + retro':'retro non assegnato'}</span><span>${model.realMeshGlb?'GLB mesh':'depth/orbit GPU'}</span><span>${esc(model.searchPolicy||'RunPod V32: segmentazione + depth + orbit frames')}</span></div></div>
        <div class="v30Card"><h4><span>Fonti + docente OpenAI</span><small>${refsStatus}</small></h4><div class="gpuTeacher"><div class="gpuMetaBox"><b>Docente OpenAI</b><div class="v30Facts"><span class="${teacher.called?'ok':'warn'}">${teacher.called?'chiamata fatta':'non chiamata'}</span><span>${esc(teacher.reason||'non necessaria')}</span></div><code>${esc(typeof teacher.result==='string'?teacher.result:JSON.stringify(teacher.result||{},null,2))}</code></div><div id="v30ReferenceList">${candidatesHtml}</div></div></div>
      </div>
      <div class="v30Card" style="margin-top:12px"><h4><span>Dati render</span><small>memoria server</small></h4><div class="kv">${row('Nome',id.productName||product.title)}${row('Marca',id.brand||product.brand)}${row('Formato',id.format||product.format)}${row('Categoria',f.classification?.category||product.category)}${row('Etichetta',labelText)}${row('Barcode',barcode||'non rilevato')}${row('Profilo', 'solo scelta manuale titolare')}${row('3D',model.mode||'virtual_3d')}</div><div class="renderActions"><button class="ghost" id="btnRenderTransparent">Apri PNG prodotto</button><button class="ghost" id="btnRenderWhite">Apri sfondo bianco</button><button class="ghost" id="btnCopyRenderSvg">Copia JSON GPU</button></div></div>
    </div>`;
  }
  ...
  function renderDetail(){const p=currentProduct();const d=$('detail');if(!p){d.innerHTML='<div class="detailBody"><div class="empty">Nessun dettaglio disponibile.</div></div>';return;}const f=p.fields||{};const src=profileSrc(p.profilePhoto);d.innerHTML=`<div class="detailTop"><img class="photo" src="${esc(src)}" alt=""><div><h2>${esc(p.title||'Prodotto')}</h2><p class="detailMeta">${esc(p.brand||'—')} · ${esc(p.format||'—')} · ${esc(p.category||'—')}</p><div class="chips"><span class="chip ${chipClass(p.reliability)}">Affidabilità ${esc(p.reliability||'bassa')}</span><span class="chip ${chipClass(p.completeness?.percent)}">Completo ${p.completeness?.percent||0}%</span>${f.ownerOverrides?.enabled?'<span class="chip lock">Valori titolare</span>':''}${f.objectFolder?.hasRealProfilePhoto?'<span class="chip ok">Foto reale</span>':'<span class="chip warn">Foto profilo da migliorare</span>'}</div><div class="actions" style="margin-top:10px"><button class="ghost mini" data-open-photos="1">📸 Cambia foto articolo</button></div></div></div><div class="tabs"><button class="tab ${state.tab==='overview'?'active':''}" data-tab="overview">Panoramica</button><button class="tab ${state.tab==='edit'?'active':''}" data-tab="edit">Modifica</button><button class="tab ${state.tab==='photos'?'active':''}" data-tab="photos">Foto</button><button class="tab ${state.tab==='render'?'active':''}" data-tab="render">Render AI</button><button class="tab ${state.tab==='data'?'active':''}" data-tab="data">Dati</button><button class="tab ${state.tab==='sources'?'active':''}" data-tab="sources">Fonti API</button><button class="tab ${state.tab==='json'?'active':''}" data-tab="json">JSON</button></div><div class="detailBody" id="detailBody"></div>`;
    ...
    d.querySelector('#btnV302ForceMarker')?.addEventListener('click',()=>alert('V31.7.1 attiva ✅ fix pagina bianca + endpoint V32'));
