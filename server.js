...
// =============================================================
// V29.4 PRO HUMAN RENDER ENGINE
// Filosofia: non fingere 3D se mancano viste. Usa foto reale + reference reale,
// crop etichetta e board 360 onesto. Migliora output visivo per invio futuro a OpenAI.
// =============================================================
function v2940Xml(s){return String(s??'').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));}
function v2940Field(rec,path,fallback=''){
  try{return path.split('.').reduce((o,k)=>o&&o[k],rec)||fallback;}catch(_){return fallback;}
}
function v2940Identity(rec={}){
  return {
    brand:String(rec.brand||v2940Field(rec,'memoryCard.identity.brand')||'').trim()||'Marca',
    name:String(rec.productName||v2940Field(rec,'memoryCard.identity.productName')||'').trim()||'Prodotto',
    format:String(rec.format||v2940Field(rec,'memoryCard.identity.format')||'').trim(),
    category:String(rec.category||v2940Field(rec,'memoryCard.classification.category')||'').trim()
  };
}
async function v2940DataUrlToImageBuffer(dataUrl, opts={}){
  const sharp=await v2864Sharp(); if(!sharp) return null;
  let buf=null;
  if(/^data:image\//i.test(String(dataUrl||''))) buf=v2864DataUrlBuffer(dataUrl);
  else if(/^https?:\/\//i.test(String(dataUrl||''))){ const d=await v2900FetchRemoteImageDataUrl(dataUrl,{timeoutMs:opts.timeoutMs||8000}); if(d) buf=v2864DataUrlBuffer(d); }
  if(!buf) return null;
  const width=opts.width||900, height=opts.height||900;
  const format=opts.format||'jpeg';
  const s=sharp(buf,{failOn:'none'}).rotate().resize({width,height,fit:opts.fit||'inside',withoutEnlargement:true});
  return format==='png'?await s.png({compressionLevel:8}).toBuffer():await s.jpeg({quality:opts.quality||91,mozjpeg:true}).toBuffer();
}
async function v2940GetReferenceDataUrl(key='',rec={}){
  try{
    let out=null;
    if(typeof v2920FindOnlineReferenceImage==='function') out=await v2920FindOnlineReferenceImage(key);
    else if(typeof v2880FindOnlineReferenceImage==='function') out=await v2880FindOnlineReferenceImage(key);
    if(out&&out.ok&&out.reference){
      const ref=out.reference;
      const src=ref.displayUrl||ref.imageDataUrl||ref.imageUrl||ref.image_front_url||ref.image||'';
      const data=/^data:image\//i.test(String(src))?src:await v2900FetchRemoteImageDataUrl(src,{timeoutMs:8500});
      if(data) return {dataUrl:data,source:ref.source||ref.sourceLabel||'Reference online/API',productName:ref.productName||ref.product_name||'',brand:ref.brand||ref.brands||''};
    }
  }catch(e){ updateGlobalLearningAudit({type:'v2940-reference-error',key,error:String(e?.message||e).slice(0,120)}); }
  return null;
}
function v2940AnglePhotos(rec={}){
  const photos=Array.isArray(rec.objectFolder?.photos)?rec.objectFolder.photos:[];
  const out={front:null,left:null,right:null,back:null,top:null};
  const add=(slot,p)=>{ if(!out[slot] && (p.dataUrl||p.externalUrl||p.imageUrl||p.url)) out[slot]=p; };
  const front=rec.objectFolder?.representativePhoto||rec.profilePhoto||rec.ownerProfilePhoto||null;
  if(front) out.front=front;
  for(const p of photos){ const k=String([p.kind,p.type,p.role,p.label,p.source].filter(Boolean).join(' ')).toLowerCase();
    if(/retro|back|rear/.test(k)) add('back',p); else if(/left|sinistra|lato sx|side_left/.test(k)) add('left',p); else if(/right|destra|lato dx|side_right/.test(k)) add('right',p); else if(/top|alto|tappo/.test(k)) add('top',p); else if(/front|fronte|profilo|product/.test(k)) add('front',p);
  }
  return out;
}
async function v2940BuildHumanCards({productBuf,labelBuf,referenceBuf,angles,rec,meta}){
  const sharp=await v2864Sharp(); if(!sharp) return {};
  const id=v2940Identity(rec); const x=v2940Xml;
  const category=id.category||meta.category||'categoria';
  const productPanel=await sharp({create:{width:760,height:760,channels:4,background:{r:255,g:255,b:255,alpha:0}}}).composite([{input:await sharp(productBuf).resize({width:720,height:700,fit:'inside',withoutEnlargement:true}).png().toBuffer(),gravity:'center'}]).png().toBuffer();
  const labelPanel=labelBuf?await sharp({create:{width:640,height:210,channels:4,background:{r:255,g:255,b:255,alpha:0}}}).composite([{input:await sharp(labelBuf).resize({width:620,height:190,fit:'inside',withoutEnlargement:true}).jpeg({quality:92,mozjpeg:true}).toBuffer(),gravity:'center'}]).png().toBuffer():null;
  const refPanel=referenceBuf?await sharp({create:{width:540,height:600,channels:4,background:{r:255,g:255,b:255,alpha:0}}}).composite([{input:await sharp(referenceBuf).resize({width:520,height:580,fit:'inside',withoutEnlargement:true}).jpeg({quality:91,mozjpeg:true}).toBuffer(),gravity:'center'}]).png().toBuffer():null;
  const studioSvg=`<svg xmlns="http://www.w3.org/2000/svg" width="1100" height="1380" viewBox="0 0 1100 1380"><defs><linearGradient id="bg" x1="0" x2="0" y1="0" y2="1"><stop stop-color="#fbfdff"/><stop offset="1" stop-color="#edf6ff"/></linearGradient><linearGradient id="pill" x1="0" x2="1"><stop stop-color="#f6d54d"/><stop offset="1" stop-color="#244596"/></linearGradient><filter id="s" x="-20%" y="-20%" width="140%" height="160%"><feDropShadow dx="0" dy="24" stdDeviation="24" flood-color="#0e2a4d" flood-opacity=".18"/></filter></defs><rect width="1100" height="1380" rx="48" fill="url(#bg)"/><rect x="44" y="44" width="1012" height="1292" rx="38" fill="#fff" stroke="#d8e7fa" stroke-width="3"/><text x="550" y="102" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="34" font-weight="1000" fill="#10233f">Render AI V29.4 · prodotto reale</text><text x="550" y="142" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="22" font-weight="900" fill="#5e728c">foto prodotto + reference + etichetta: output pulito per analisi futura</text><rect x="94" y="900" width="912" height="140" rx="32" fill="url(#pill)"/><text x="550" y="960" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="38" font-weight="1000" fill="#fff">${x(id.brand)} · ${x(id.name)}</text><text x="550" y="1005" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="24" font-weight="900" fill="#f3f7ff">${x(id.format)} · ${x(category)}</text><rect x="130" y="1078" width="840" height="170" rx="28" fill="#f7fbff" stroke="#dce9f7"/><text x="550" y="1138" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="25" font-weight="1000" fill="#10233f">Etichetta OCR-ready · forma reale · valori titolare</text><text x="550" y="1182" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="20" font-weight="900" fill="#5e728c">${x(meta.note||'render guidato da pixel reali, senza 3D finto')}</text></svg>`;
  const comps=[{input:productPanel,left:170,top:170}]; if(labelPanel) comps.push({input:labelPanel,left:230,top:670});
  const studio=await sharp(Buffer.from(studioSvg)).composite(comps).png({compressionLevel:8}).toBuffer();
  const semanticSvg=`<svg xmlns="http://www.w3.org/2000/svg" width="1180" height="1320" viewBox="0 0 1180 1320"><defs><linearGradient id="bg" x1="0" x2="1"><stop stop-color="#f8fbff"/><stop offset="1" stop-color="#eef6ff"/></linearGradient><linearGradient id="pill" x1="0" x2="1"><stop stop-color="#f6d54d"/><stop offset="1" stop-color="#244596"/></linearGradient></defs><rect width="1180" height="1320" rx="48" fill="url(#bg)"/><rect x="44" y="44" width="1092" height="1232" rx="38" fill="#fff" stroke="#d8e7fa" stroke-width="3"/><text x="74" y="104" font-family="Inter,Arial,sans-serif" font-size="34" font-weight="1000" fill="#10233f">Gemello semantico PRO</text><text x="1106" y="104" text-anchor="end" font-family="Inter,Arial,sans-serif" font-size="23" font-weight="900" fill="#62758e">umano · pixel · reference</text><rect x="80" y="870" width="1020" height="120" rx="32" fill="url(#pill)"/><text x="590" y="925" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="34" font-weight="1000" fill="#fff">${x(id.brand)} · ${x(id.name)} · ${x(id.format)}</text><text x="590" y="964" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="21" font-weight="900" fill="#f4f8ff">soggetto reale + etichetta + reference online se disponibile</text><rect x="80" y="1030" width="1020" height="170" rx="26" fill="#f8fbff" stroke="#dce9f7"/><text x="590" y="1096" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="24" font-weight="1000" fill="#10233f">360 reale richiede frontale, lati e retro. Con una foto: riconoscimento frontale PRO, non modello 3D.</text><text x="590" y="1142" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="20" font-weight="900" fill="#60758f">Il server ora non disegna forme inventate: mostra dati veri e slot mancanti.</text></svg>`;
  const semComps=[{input:await sharp(productBuf).resize({width:470,height:610,fit:'inside',withoutEnlargement:true}).jpeg({quality:91,mozjpeg:true}).toBuffer(),left:120,top:170}];
  if(refPanel) semComps.push({input:refPanel,left:610,top:170});
  if(labelPanel) semComps.push({input:labelPanel,left:270,top:670});
  const semantic=await sharp(Buffer.from(semanticSvg)).composite(semComps).png({compressionLevel:8}).toBuffer();
  const frontBuf=await sharp(productBuf).resize({width:280,height:420,fit:'inside',withoutEnlargement:true}).jpeg({quality:90,mozjpeg:true}).toBuffer();
  const refBuf=referenceBuf?await sharp(referenceBuf).resize({width:280,height:420,fit:'inside',withoutEnlargement:true}).jpeg({quality:90,mozjpeg:true}).toBuffer():null;
  const boardSvg=`<svg xmlns="http://www.w3.org/2000/svg" width="1250" height="940" viewBox="0 0 1250 940"><defs><linearGradient id="bg" x1="0" x2="1"><stop stop-color="#f8fbff"/><stop offset="1" stop-color="#eef6ff"/></linearGradient><linearGradient id="pill" x1="0" x2="1"><stop stop-color="#f6d54d"/><stop offset="1" stop-color="#244596"/></linearGradient></defs><rect width="1250" height="940" rx="44" fill="url(#bg)"/><rect x="40" y="40" width="1170" height="860" rx="36" fill="#fff" stroke="#d8e7fa" stroke-width="3"/><text x="625" y="96" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="34" font-weight="1000" fill="#10233f">360° PRO · ricostruzione onesta</text><text x="625" y="134" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="21" font-weight="900" fill="#62758e">fronte reale + reference. Lati/retro diventano reali solo quando aggiungi foto dedicate.</text><rect x="76" y="180" width="260" height="460" rx="28" fill="#f8fbff" stroke="#dce9f7"/><rect x="360" y="180" width="260" height="460" rx="28" fill="#f8fbff" stroke="#dce9f7"/><rect x="644" y="180" width="260" height="460" rx="28" fill="#fff8ed" stroke="#f2d69c" stroke-dasharray="12 8"/><rect x="928" y="180" width="260" height="460" rx="28" fill="#fff8ed" stroke="#f2d69c" stroke-dasharray="12 8"/><text x="206" y="224" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="22" font-weight="1000" fill="#10233f">FRONTE</text><text x="490" y="224" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="22" font-weight="1000" fill="#10233f">REFERENCE</text><text x="774" y="224" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="22" font-weight="1000" fill="#8a6400">LATO MANCANTE</text><text x="1058" y="224" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="22" font-weight="1000" fill="#8a6400">RETRO MANCANTE</text><text x="774" y="425" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="23" font-weight="1000" fill="#8a6400">scansiona lato</text><text x="774" y="462" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="18" font-weight="900" fill="#a57b18">per 360 vero</text><text x="1058" y="425" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="23" font-weight="1000" fill="#8a6400">scansiona retro</text><text x="1058" y="462" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="18" font-weight="900" fill="#a57b18">per 360 vero</text><rect x="120" y="720" width="1010" height="92" rx="28" fill="url(#pill)"/><text x="625" y="776" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="30" font-weight="1000" fill="#fff">${x(id.brand)} · ${x(id.name)} · ${x(id.format)}</text><text x="625" y="860" text-anchor="middle" font-family="Inter,Arial,sans-serif" font-size="18" font-weight="900" fill="#62758e">questa non è una finta rotazione: è una board per capire cosa manca alla memoria visiva.</text></svg>`;
  const bComps=[{input:frontBuf,left:66,top:245}]; if(refBuf) bComps.push({input:refBuf,left:350,top:245});
  const board=await sharp(Buffer.from(boardSvg)).composite(bComps).png({compressionLevel:8}).toBuffer();
  return {studio,semantic,board};
}
async function v2940GenerateRealPixelRender(key='',opts={}){
  try{
    ensureDbShape(); const rec=db.assistantBrain?.globalProductMemory?.products?.[key]; if(!rec) return {ok:false,error:'product_not_found'};
    const photo=v2879PickRenderPhoto(rec); if(!photo) return {ok:false,error:'no_real_photo'}; const sharp=await v2864Sharp(); if(!sharp) return {ok:false,error:'sharp_not_available'};
    const reference=await v2940GetReferenceDataUrl(key,rec).catch(()=>null);
    const raw=photo.dataUrl||reference?.dataUrl||photo.externalUrl||photo.imageUrl||'';
    const baseBuf=await v2940DataUrlToImageBuffer(raw,{width:1100,height:1100,fit:'inside',quality:92}); if(!baseBuf) return {ok:false,error:'image_source_failed'};
    const refBuf=reference?.dataUrl?await v2940DataUrlToImageBuffer(reference.dataUrl,{width:1000,height:1000,fit:'inside',quality:92}):null;
    const meta=await sharp(baseBuf,{failOn:'none'}).metadata(); const w=meta.width||900,h=meta.height||900;
    let pbox=v2930SafeBox(w,h,rec), lbox=v2930LabelBox(pbox,w,h,rec);
    try{
      const img=sharp(baseBuf,{failOn:'none'}).ensureAlpha(); const {data,info}=await img.raw().toBuffer({resolveWithObject:true});
      const smart=typeof v2900BuildSmartMask==='function'?v2900BuildSmartMask(data,info.width,info.height,rec):null;
      if(smart&&smart.mask){ const b=typeof v2900RefineProductBox==='function'?v2900RefineProductBox(smart.mask,info.width,info.height,rec):v2930SafeBox(info.width,info.height,rec); const lb=typeof v2920DetectLabelBox==='function'?v2920DetectLabelBox(data,info.width,info.height,b,rec):v2930LabelBox(b,info.width,info.height,rec); pbox=b; lbox=lb; }
    }catch(_){ }
    const productBuf=await v2930ExtractJpeg(sharp,baseBuf,pbox,{height:920,width:760,fit:'inside',withoutEnlargement:true});
    const labelBuf=await v2930ExtractJpeg(sharp,baseBuf,lbox,{width:820,height:300,fit:'inside',withoutEnlargement:true});
    const cards=await v2940BuildHumanCards({productBuf,labelBuf,referenceBuf:refBuf,angles:v2940AnglePhotos(rec),rec,meta:{category:String(rec.category||rec.memoryCard?.classification?.category||'').trim(),note:reference?'reference online + pixel reali':'pixel reali + crop etichetta'}});
    const qualityScore=reference?91:84;
    rec.realPixelRenderV2940={at:Date.now(),photoId:photo.id||'',kind:photo.kind||'',reference:reference?{source:reference.source,productName:reference.productName,brand:reference.brand}:null,labelBox:{x:Math.round(lbox.minX),y:Math.round(lbox.minY),w:Math.round(lbox.maxX-lbox.minX+1),h:Math.round(lbox.maxY-lbox.minY+1),confidence:Math.round(lbox.confidence||82)},engine:'v29_4_human_photo_reference_board'};
    return {ok:true,version:'V29.4',mode:'human_real_reference_render',source:{id:photo.id||'',kind:photo.kind||'',score:photo.score||0},reference:reference||null,render:{masterDataUrl:v2879DataUrlFromBuffer(baseBuf,'image/jpeg'),whiteDataUrl:v2879DataUrlFromBuffer(productBuf,'image/jpeg'),transparentDataUrl:v2879DataUrlFromBuffer(productBuf,'image/jpeg'),studioDataUrl:v2879DataUrlFromBuffer(cards.studio,'image/png'),semanticDataUrl:v2879DataUrlFromBuffer(cards.semantic,'image/png'),render360DataUrl:v2879DataUrlFromBuffer(cards.board,'image/png'),labelCropDataUrl:v2879DataUrlFromBuffer(labelBuf,'image/jpeg'),labelBox:rec.realPixelRenderV2940.labelBox,quality:{level:reference?'pro_reference_human':'pro_human_photo',score:qualityScore,labelConfidence:rec.realPixelRenderV2940.labelBox.confidence,message:reference?'V29.4: render umano con reference online reale + pixel prodotto':'V29.4: render umano da foto reale, 360 onesto senza lato/retro finti'}}};
  }catch(e){ return {ok:true,version:'V29.4',mode:'safe_human_fallback',render:{quality:{level:'safe_fallback',score:45,message:'Fallback V29.4: '+String(e?.message||e).slice(0,120)}}}; }
}
try{ v2879GenerateRealPixelRender=v2940GenerateRealPixelRender; }catch(_){ }
(function(){ const V='V29.4'; try{ const prev=publicServerBrainV2840; if(typeof prev==='function'&&!global.__v2940ServerBrainWrapped){ publicServerBrainV2840=function(opts={}){ const out=prev.call(this,opts||{})||{}; try{ out.version='V29.4 PRO HUMAN RENDER ENGINE'; out.reasoningBusV2940={active:true,policy:'render umano: foto reale + reference reale + etichetta + board 360 onesta, niente 3D finto'}; }catch(_){} return out;}; global.__v2940ServerBrainWrapped=true; }}catch(_){} try{ const prev=preflightSnapshotV98; if(typeof prev==='function'&&!global.__v2940PreflightWrapped){ preflightSnapshotV98=function(){ const s=prev.call(this)||{}; s.version=V; s.brain=Object.assign({},s.brain||{},{version:V,humanRender:'active',honest360:'active',referenceFusion:'active'}); return s;}; global.__v2940PreflightWrapped=true; }}catch(_){} console.log('[Spesa Pronta] V29.4 PRO HUMAN RENDER ENGINE active');})();
