window.SPESA_PRONTA_VERSION='v27.21-ultra-learning';
...
let liveScanStream = null, liveScanTimer = null, liveScanBusy = false, liveScanActive = false, liveScanStableCount = 0, liveScanLastHint = '', liveScanLastMetrics = null;
...
function openGroceryScanner(afterShopping=false){
  ...
  $('#scannerStatus').textContent = !settings.inventorySetupDone ? 'Inventario iniziale: usa la diretta AI o scatta una foto. La Vision AI prova a leggere marca, forma, scadenza, quantità e stato del prodotto prima di confermare.' : (afterShopping ? 'Hai premuto “Ho fatto la spesa”. Ora puoi usare la diretta AI: quando il prodotto entra bene nell’inquadratura, provo a scattare in automatico e a leggere marca, scadenza, quantità, categoria e danni.' : 'Metti un articolo davanti alla videocamera o carica una foto. La Vision AI ragiona su marca, forma, scadenza, tipologia, quantità e possibili danni.');
  try{ dlg.showModal(); }catch{ dlg.setAttribute('open',''); }
  openAiPanel();
}
function stopLiveVisionMode(keepMessage=false){
  liveScanActive=false; liveScanStableCount=0; liveScanLastHint='';
...
function resetScannerResults(){ stopLiveVisionMode(true); $('#scannerResults').innerHTML=''; $('#scannerPreview').innerHTML=''; $('#scannerStatus').textContent='Risultati svuotati. Puoi riaprire la diretta AI o scattare nuove foto.'; }
...
function finishScanner(){
  ...
  if(!settings.inventorySetupDone && confirmed===0){
    $('#scannerStatus').textContent='Per completare il primo inventario devi confermare almeno un prodotto scansionato.';
...
function renderLiveScanStage(note='Metti il prodotto nel riquadro: quando è ben centrato provo a scattare da solo.'){
  ...
  <p class="live-scan-note">${esc(note)}</p>`;
  $('#scannerCaptureNowInlineBtn')?.addEventListener('click', ()=>captureLiveFrame(true));
  $('#scannerStopLiveBtn')?.addEventListener('click', ()=>{ stopLiveVisionMode(); $('#scannerStatus').textContent='Diretta chiusa. Puoi riaprirla quando vuoi.'; });
}
async function startLiveVisionMode(mode='smart'){
  ...
  try{
    ...
    liveScanActive=true; liveScanStableCount=0; liveScanBusy=false;
    $('#scannerStatus').textContent='Diretta AI attiva: mostra un solo prodotto per volta, centrato e con la scadenza visibile se possibile.';
    queueLiveScanLoop();
  }catch(err){
    stopLiveVisionMode(true);
    $('#scannerStatus').textContent='Non riesco ad aprire la videocamera. Puoi comunque scattare o caricare una foto.';
...
function analyzeLiveFrameMetrics(video){
  ...
  for(let i=0,p=0;i<d.length;i+=4,p++){
    const l=.2126*d[i]+.7152*d[i+1]+.0722*d[i+2]; lum[p]=l; sum+=l;
  }
  const avg=sum/lum.length;
  let edgeCenter=0, edgeOuter=0, countCenter=0, countOuter=0;
  let varSum=0;
  for(let y=1;y<96;y++){
    for(let x=1;x<96;x++){
      const idx=y*96+x, a=lum[idx], b=lum[idx-1], c=lum[idx-96];
      const diff=Math.abs(a-b)+Math.abs(a-c);
      const inCenter = x>26 && x<70 && y>22 && y<74;
      if(inCenter){ edgeCenter+=diff; countCenter++; }
      else { edgeOuter+=diff; countOuter++; }
      varSum+=(a-avg)*(a-avg);
    }
  }
  const contrast=Math.sqrt(varSum/lum.length);
  edgeCenter/=Math.max(1,countCenter);
  edgeOuter/=Math.max(1,countOuter);
  const centered=edgeCenter>8.5 && edgeCenter>edgeOuter*1.14;
  const readable=avg>45 && avg<220 && contrast>16 && edgeCenter>7.4;
  const stable=true;
  return {avg,contrast,edgeCenter,edgeOuter,centered,readable,stable};
}
function updateLiveHud(metrics, ready){
  ...
  if(metrics.avg<=45) hint='Serve più luce';
  else if(metrics.avg>=220) hint='Troppa luce o riflesso';
  else if(metrics.contrast<=16) hint='Prodotto poco leggibile';
  else if(!metrics.centered) hint='Mettilo dentro il riquadro';
  else hint='Perfetto, resto un attimo e scatto';
  main.textContent=hint;
  auto.textContent=ready ? 'Auto-scatto imminente' : `Qualità ${Math.max(1,Math.round((metrics.contrast+metrics.edgeCenter)*2))}%`;
  main.className='live-scan-pill'+(ready?' good':'');
  auto.className='live-scan-pill'+(ready?' good':' warn');
  guides.className='live-scan-guides'+(ready?' ready':'');
}
async function captureLiveFrame(manual=false){
  ...
  $('#scannerStatus').textContent = manual ? 'Sto analizzando lo scatto manuale...' : 'Prodotto centrato: scatto automatico in corso, sto leggendo marca, scadenza, quantità e stato.';
...
async function analyzeGroceryDataUrl(dataUrl,fileName='photo.jpg'){
  ...
  if(!liveStage) $('#scannerPreview').innerHTML=`<img src="${dataUrl}" alt="Foto articolo"><p>Vision AI sta leggendo marca, tipo, quantità, scadenza e stato del prodotto...</p>`;
  ...
  if(!quality.ok){
    $('#scannerStatus').textContent=`Non vedo bene: ${quality.reason}. Rifai la foto più vicino, con luce buona, prodotto centrato e data di scadenza visibile.`;
  ...
  if(result.needsRetake){ $('#scannerStatus').textContent=result.reason || 'Non vedo bene, rifai la foto.'; addScannerResult({...result,dataUrl}); document.querySelector('.scanning-preview')?.remove(); return; }
  result.dataUrl=dataUrl; result.quality=quality;
  const summary=presentVisionResults(result,dataUrl);
  if(summary.count>1){
    $('#scannerStatus').textContent=`Ho rilevato ${summary.count} prodotti nella scena. ${summary.autoCount?summary.autoCount+' aggiunti automaticamente, ':''}${summary.manualCount?summary.manualCount+' da confermare manualmente.':'nessuno da confermare.'}`;
  }else{
    $('#scannerStatus').textContent=(result.needsManual?'Controllo manuale: completa i dati, soprattutto nome, quantità e scadenza.':'Vision AI completata: ho letto prodotto, quantità e ciò che vedo della confezione. Controlla e conferma: ogni conferma mi aiuta a diventare più precisa con i tuoi prodotti.');
  }
  document.querySelector('.scanning-preview')?.remove();
}
...
function confirmScanResult(el,result,silent=false){
  ...
  saveAiMemory(); saveAll(); render(); if(!silent) toast('Articolo aggiornato in casa ✅');
}
