window.SPESA_PRONTA_VERSION='v27.17-photo-recognition-fix';
...
function addScannerResult(result){
  ...
  const html=`<article class="scan-result ${result.needsRetake?'bad':''}" id="${id}">
    ...
    <div class="scan-fields">
      <strong>${result.needsRetake?'Foto da rifare':(result.needsManual?'Prodotto da verificare':'Prodotto riconosciuto')}</strong>
      ...
      <label>Nome prodotto<input data-scan-name value="${esc(result.productName||'')}" placeholder="${esc(result.productPlaceholder||'Es. Coca-Cola, latte, acqua...')}"></label>
