window.SPESA_PRONTA_VERSION='v27.28-max-pro';
...
function getScanCompletionStatus(el, result={}){
  ...
  const checks=[
    {key:'name', label:'Nome', ok:!!s.name && !/^es\./i.test(s.name), warn:Number(result.confidence||0)<0.55},
    {key:'brand', label:'Marca', ok:!!s.brand || !!result.brand, warn:Number(result.confidence||0)<0.68 && !s.brand},
    {key:'qty', label:'Quantità', ok:s.qty>0 && !!s.unit},
    {key:'expiry', label:'Scadenza', ok:!!s.expiry, warn:!s.expiry},
    {key:'category', label:'Categoria', ok:!!s.category},
    {key:'damage', label:'Stato', ok:!!s.damage}
  ];
  const done=checks.filter(c=>c.ok).length;
  return {state:s, checks, done, total:checks.length, missing:checks.filter(c=>!c.ok).map(c=>c.key), warn:checks.filter(c=>c.warn && !c.ok).map(c=>c.key)};
}
...
function renderScanCompletionStatus(el, result={}){
  ...
  const line = info.missing.length ? `Completamento ${pct}%. Mancano: ${info.missing.map(k=>({name:'nome',brand:'marca',qty:'quantità',expiry:'scadenza',category:'categoria',damage:'stato'}[k]||k)).join(', ')}.` : 'Scheda completa: puoi confermare.';
...
function parseNameFromSpeech(raw=''){
  return String(raw||'').replace(/^(no[, ]*)?(non e|non è|e|è|si chiama|nome|prodotto|correggi il nome in|correggi nome in|metti nome)\s+/i,'').replace(/^(di marca)\s+/i,'').trim();
}
function parseBrandFromSpeech(raw=''){
  const direct=String(raw||'').match(/(?:marca|brand|di marca)\s+(.+)/i);
...
function getNextScanMicStep(el, result={}){
  ...
  if((!state.brand && Number(result.confidence||0)<0.68) || (!state.brand && result.brand && !el.dataset.voiceBrandDone)) return 'brand';
  if(!(state.qty>0)) return 'qty';
  if(result.isDamaged && !el.dataset.voiceDamageDone) return 'damage';
  if(!state.expiry && !el.dataset.voiceExpiryDone) return 'expiry';
...
function scanMicPromptForStep(step, el, result={}){
  ...
  return 'La scheda è pronta. Puoi dire riepilogo per sentire tutto, conferma per aggiungerlo in casa, oppure dimmi cosa correggere.';
}
...
function applyScannerMicAnswer(el, result={}, raw=''){
  ...
  const explicitName = current==='name' || /(^|\b)(non e|non è|si chiama|nome|prodotto|correggi.*nome|metti nome)(\b)/.test(norm);
...
function addScannerResult(result){
  ...
  const html=`<article class="scan-result ${result.needsRetake?'bad':''} ${result.needsManual?'manual':'ai-ok'}" id="${id}">
    ...
    <div class="scan-fields">
      ...
      <div class="scan-actions-row"><button class="secondary-btn" type="button" data-scan-recap>🧠 Riepilogo AI</button><button class="primary-btn" data-confirm-scan>Conferma e aggiungi in casa</button></div>`}
    </div>
  </article>`;
