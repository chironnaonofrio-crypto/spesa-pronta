window.SPESA_PRONTA_VERSION='v27.20-learning-vision';
...
function rememberLearnedProduct(payload={}){
  ...
  if(!row){
    row={ key, productName:name, brand, variant, category, unit, productType:String(payload.productType||''), packageType:String(payload.packageType||''), estimatedSize:String(payload.estimatedSize||''), isLiquid:!!payload.isLiquid, seenCount:0, lastConfirmedAt:0, aliases:[] };
...
function confirmScanResult(el,result){
  ...
  rememberLearnedProduct({productName,brand:result.brand||'',variant:result.variant||'',category,unit,productType:result.productType||'',packageType:result.packageType||'',estimatedSize:result.estimatedSize||'',isLiquid:!!result.isLiquid,baseName:result.productName||''});
