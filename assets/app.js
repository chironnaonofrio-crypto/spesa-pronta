...
async function captureLiveFrame(manual=false){
  ...
  try{
    ...
    try{ logAiDiagnosticV98?.('live-capture-analysis-error-v2832',{message:String(err?.message||err).slice(0,180)}); }catch(_){ }
...
async function analyzeGroceryDataUrl(dataUrl,fileName='photo.jpg', visualSignature=''){
  ...
  if(localPre && localAutonomyReady(localPre)){
    ...
    if(cloudActuallyFailed){
      ...
      result.cloudError=teacherNeededByLocal ? (String(visionError?.message||result?.cloudError||'openai_not_available').slice(0,80)) : '';
      if(!result.reason || /Cloud OpenAI|Failed to fetch|cloud_|risposta non valida|Docente OpenAI/i.test(String(result.reason||''))) result.reason=teacherNeededByLocal ? 'Il colore/forma non basta: non classifico a caso. Serve etichetta leggibile oppure docente OpenAI attivo.' : 'Analisi server/local-first completata: controlla i dati e conferma.';
    } else {
...
async function askVisionAi(dataUrl, opts={}){
  // V28.32: server-first reale. Non bloccare più la scansione solo perché /status dice
  // che il docente OpenAI non è pronto: il server può comunque usare memoria/cache/regole
  // e, se la chiave è presente, userà OpenAI come docente finale.
  ...
  const endpoint=visionApiBase();
  const controller = typeof AbortController!=='undefined' ? new AbortController() : null;
  const timer = controller ? setTimeout(()=>controller.abort(), String(opts?.stage||'auto').toLowerCase()==='expiry' ? 12000 : 18000) : null;
  let res;
  try{
    ...
    e.visionError={error:e.message, detail:String(fetchErr?.message||fetchErr), status:st||null};
    throw e;
  }finally{ if(timer) clearTimeout(timer); }
  if(!res.ok){
  ...
  if(r && r.cloudVision) { r.cloudVision=true; r.cloudOffline=false; }
  if(r && !r.cloudVision && !r.teacherInactive){ r.cloudOffline=false; }
...
// V28.54 client marker: PRO cost meter + semantic signature active on server
