window.SPESA_PRONTA_VERSION='v27.24-complete-assistant';
...
function resetScannerResults(){ stopLiveVisionMode(true); $('#scannerResults').innerHTML=''; $('#scannerPreview').innerHTML=''; setScannerStatus('Risultati svuotati. Puoi riaprire la diretta AI o scattare nuove foto.', '', false); }
...
function renderLiveScanStage(note='Metti il prodotto nel riquadro: quando è ben centrato provo a scattare da solo.'){
  ...
  <div class="live-scan-controls"><button class="primary-btn" id="scannerCaptureNowInlineBtn" type="button">⚡ Scatta adesso</button><button class="mini-btn" id="scannerVoiceToggleBtn" type="button">🔊 Voce attiva</button><button class="outline-btn" id="scannerStopLiveBtn" type="button">Chiudi diretta</button></div>
  <p class="live-scan-note"><strong>Assistente vocale attivo.</strong> <em>Ti guida passo passo</em> e ti dice come muovere il prodotto. ${esc(note)}</p>`;
...
function confirmScanResult(el,result,silent=false){
  ...
  saveAiMemory(); saveAll(); render(); if(!silent){ toast('Articolo aggiornato in casa ✅'); const finalVoice=`Perfetto. Ho aggiornato ${productName} in casa.${expiryDate? ' Scadenza registrata '+expiryDate+'.' : ''}${damageNote && damageNote!=='Integro' ? ' Ho segnato anche lo stato del prodotto.' : ''}`; speakNatural(finalVoice, {flush:true, rate:1.0, pitch:1.02}); }
}
