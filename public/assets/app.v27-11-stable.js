window.SPESA_PRONTA_VERSION='v27.11-inline-icons-no-broken-images';
// V27.11: stop reload loop. Clean old caches/service workers only once, without reloading the page.
(function(){
  try{
    const flag='spesaProntaNoLoopCleanedV2711';
...
function newCaptcha(){
  const pool=[
    ...
    {id:'dogfood', label:'Crocchette'},
    {id:'key', label:'Chiave'},
    {id:'lock', label:'Lucchetto'},
...
function renderCaptcha(){
  ...
  box.innerHTML=captcha.options.map(o=>`<button class="captcha-choice" type="button" data-captcha="${esc(o.id)}" aria-label="${esc(o.label)}"><span class="captcha-visual-icon captcha-icon-${esc(o.id)}" aria-hidden="true"></span><span class="captcha-label">${esc(o.label)}</span></button>`).join('');
...
function init(){
  ...
  // V27.11: service worker disabled to prevent auto-refresh loop.
