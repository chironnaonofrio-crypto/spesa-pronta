window.SPESA_PRONTA_VERSION='v27.26-scan-fix';
...
let liveScanStream = null, liveScanTimer = null, liveScanBusy = false, liveScanActive = false, liveScanStableCount = 0, liveScanLastHint = '', liveScanLastMetrics = null, liveScanPrevSample = null, liveScanLastSpeechKey='', liveScanLastSpeechAt=0, liveScanSpeechEnabled=true, scannerMicEnabled=true, scannerMicListening=false, scannerMicRecognition=null, scannerMicShouldRestart=false, scannerMicCurrentResultId='', scannerMicStep='', scannerMicLastPrompt='', liveScanReadySince=0, liveScanCooldownUntil=0, liveScanPendingResult=false;
...
function parseSpokenNumber(text=''){
  ...
  for(const w of words){ if(map[w]!==undefined) return map[w]; }
...
function parseQuantityUnitFromSpeech(text=''){
  ...
  let unit='';
  if(/bottigli/.test(n)) unit='bt';
  else if(/litr|lt/.test(n)) unit='lt';
  else if(/chil|kg/.test(n)) unit='kg';
  else if(/gramm|gr/.test(n)) unit='g';
  else if(/pezz|pz|pacch|confezion/.test(n)) unit='pz';
...
function getNextScanMicStep(el, result={}){
  const name=el.querySelector('[data-scan-name]')?.value?.trim()||'';
  const qty=Number(String(el.querySelector('[data-scan-qty]')?.value||'').replace(',','.'));
  const expiry=el.querySelector('[data-scan-expiry]')?.value?.trim()||'';
  const damage=el.querySelector('[data-scan-damage]')?.value?.trim()||'';
  const cat=el.querySelector('[data-scan-cat]')?.value?.trim()||'';
  if(!name || /^nome prodotto$/i.test(name) || /^es\./i.test(name) || (!el.dataset.voiceNameDone && Number(result.confidence||0)<0.45)) return 'name';
  if(!(qty>0)) return 'qty';
  if(result.isDamaged && !el.dataset.voiceDamageDone) return 'damage';
  if(!expiry && !el.dataset.voiceExpiryDone) return 'expiry';
...
function scanMicPromptForStep(step, el, result={}){
  ...
  if(step==='damage') return `Attenzione, vedo una possibile irregolarità: ${damageType}. Dimmi se è integro, ammaccato, rotto, aperto o con perdita.`;
  ...
  return 'La scheda è pronta. Di conferma per aggiungerlo in casa, oppure dimmi cosa correggere.';
}
...
function handleScannerMicText(text=''){
  ...
  if(/^(ripeti|aiuto|cosa devo fare)/.test(norm)){ const el=getActiveScannerResult(); if(el) promptScannerConversation(el, el._scanResult||{}, true); else if(liveScanSpeechEnabled) speakNatural('Mostrami un prodotto nel riquadro o scatta una foto, poi ti guiderò.', {flush:true}); return; }
  ...
  if(!el){ if(liveScanSpeechEnabled) speakNatural('Ti ascolto. Inquadra un prodotto o scatta una foto per iniziare la compilazione.', {flush:true}); return; }
  setActiveScannerResult(el);
  const result=el._scanResult||{};
  if(/(rifai|nuova foto|ripeti foto)/.test(norm)){ el.querySelector('[data-retake]')?.click(); return; }
  if(/(conferma|aggiungi|va bene|ok conferma|tutto ok)/.test(norm) && getNextScanMicStep(el,result)==='confirm'){ confirmScanResult(el,result); return; }
...
function applyScannerMicAnswer(el, result={}, raw=''){
  ...
  if(current==='name' || /^nome/.test(norm)){
    const cleaned=raw.replace(/^(nome|si chiama|e|è)\s+/i,'').trim();
    if(cleaned && cleaned.length>1){ setScanFieldFromVoice(el,'[data-scan-name]', cleaned); el.dataset.voiceNameDone='1'; setScanVoiceHelper(el, `Nome aggiornato: ${cleaned}.`, 'live'); return true; }
  }
  const qtyInfo=parseQuantityUnitFromSpeech(raw);
  if((current==='qty' || /quantit|pezz|bottigli|litri|chili|gramm|unit/.test(norm)) && qtyInfo.quantity!=null){
  ...
  if(current==='expiry' || /scad/.test(norm)){
    if(/salta|non so|non la so|nessuna/.test(norm)){ el.dataset.voiceExpiryDone='1'; setScanVoiceHelper(el, 'Scadenza saltata per ora.', 'live'); return true; }
    if(expiry){ setScanFieldFromVoice(el,'[data-scan-expiry]', expiry); el.dataset.voiceExpiryDone='1'; setScanVoiceHelper(el, `Scadenza aggiornata: ${expiry}.`, 'live'); return true; }
  }
  const damage=detectDamageState(raw);
  if(current==='damage' || /integr|rott|ammacc|apert|bucat|perdit|schiacci|rovinat|scadut/.test(norm)){
    if(damage){ setScanFieldFromVoice(el,'[data-scan-damage]', damage); el.dataset.voiceDamageDone='1'; result.isDamaged = damage!=='Integro'; result.damageType = damage==='Integro' ? '' : damage; setScanVoiceHelper(el, `Stato prodotto aggiornato: ${damage}.`, damage==='Integro' ? 'live' : 'warn'); return true; }
  }
  const cat=detectCategoryFromSpeech(raw);
  if(current==='category' || /categori|bevand|aliment|frutt|verdur|animal|casa|farmac|acquar/.test(norm)){
    if(cat){ setScanFieldFromVoice(el,'[data-scan-cat]', cat); setScanVoiceHelper(el, `Categoria aggiornata: ${catName(cat)}.`, 'live'); return true; }
  }
...
function stopLiveVisionMode(keepMessage=false){
  liveScanActive=false; liveScanStableCount=0; liveScanLastHint=''; liveScanPrevSample=null; liveScanLastSpeechKey=''; liveScanReadySince=0; liveScanCooldownUntil=0; liveScanPendingResult=false; stopScannerMic(true); if('speechSynthesis' in window) try{ speechSynthesis.cancel(); }catch{}
...
function resetScannerResults(){ stopLiveVisionMode(true); scannerMicCurrentResultId=''; scannerMicStep=''; scannerMicLastPrompt=''; liveScanPendingResult=false; liveScanCooldownUntil=0; $('#scannerResults').innerHTML=''; $('#scannerPreview').innerHTML=''; setScannerStatus('Risultati svuotati. Puoi riaprire la diretta AI o scattare nuove foto.', '', false); }
...
function renderLiveScanStage(note='Metti il prodotto nel riquadro: quando è ben centrato provo a scattare da solo.'){
  ...
  <div class="live-scan-controls"><button class="primary-btn" id="scannerCaptureNowInlineBtn" type="button">⚡ Scatta adesso</button><button class="mini-btn" id="scannerVoiceToggleBtn" type="button">🔊 Voce attiva</button><button class="mini-btn mic" id="scannerMicToggleBtn" type="button">🎙️ Mic attivo</button><button class="outline-btn" id="scannerStopLiveBtn" type="button">Chiudi diretta</button></div>
  <p class="live-scan-note"><strong>Assistente vocale + microfono attivi.</strong> <em>Puoi parlare con la Vision AI</em> mentre inquadri il prodotto e fino al completamento della scheda. ${esc(note)}</p>`;
  ...
  setVoiceToggleUi(); setMicToggleUi();
...
async function startLiveVisionMode(mode='smart'){
  ...
  try{
    ...
    liveScanActive=true; liveScanStableCount=0; liveScanBusy=false; liveScanPrevSample=null; liveScanLastSpeechKey=''; liveScanLastSpeechAt=0; liveScanReadySince=0; liveScanCooldownUntil=0; liveScanPendingResult=false;
...
async function runLiveScanLoop(){
  ...
  if(ready){
    ...
    if(auto && holdMs < 2200){
      ...
      auto.className='live-scan-pill warn';
    }
...
function analyzeLiveFrameMetrics(video){
  ...
  return {avg,contrast,edgeCenter,edgeOuter,centered,readable,stable,motion,objectCoverage,tooSmall,tooBig,offsetX,offsetY};
}
...
async function captureLiveFrame(manual=false){
  ...
  await analyzeGroceryDataUrl(compressed, manual?'manual-live.jpg':'auto-live.jpg');
...
async function analyzeGroceryDataUrl(dataUrl,fileName='photo.jpg'){
  ...
  if(!result || result.needsManual){ result=await guessScanFallback(fileName,dataUrl,result); }
...
function addScannerResult(result){
  ...
  const helperText=result.needsRetake ? 'Non è leggibile bene. Rifai la foto o riparti dalla diretta.' : (result.isDamaged ? `Attenzione: vedo una possibile irregolarità${result.damageType?` (${result.damageType})`:''}.` : 'Posso parlare con te fino al completamento della scheda.');
  ...
  const html=`<article class="scan-result ${result.needsRetake?'bad':''} ${result.needsManual?'manual':'ai-ok'}" id="${id}">
    ...
    <div class="scan-fields">
      ...
      <div class="scan-grid-3"><label><small>Quantità</small><input data-scan-qty type="number" min="0" step="0.1" value="${esc(result.quantity||1)}"></label><label><small>Unità</small><input data-scan-unit value="${esc(result.unit||'pz')}"></label><label><small>Scadenza</small><input data-scan-expiry value="${esc(result.expiryDate||'')}" placeholder="Es. 12/08/2026"></label></div>
      <div class="scan-meta-grid"><label><small>Categoria</small><select data-scan-cat>${categoryOptions(result.category||'food')}</select></label><label><small>Stato prodotto</small><input data-scan-damage value="${esc(result.isDamaged?(result.damageType||'Danneggiato'):'Integro')}" placeholder="Integro / rotto / ammaccato"></label></div>
  ...
  if(!result.needsRetake){ liveScanPendingResult=true; setActiveScannerResult(el); if(scannerMicEnabled && !scannerMicListening) startScannerMic(false); setTimeout(()=>promptScannerConversation(el,result,true), 450); }
  else { liveScanPendingResult=false; liveScanCooldownUntil=Date.now()+2000; if(liveScanSpeechEnabled) speakNatural('La foto è da rifare. Posso aiutarti a riprovare dalla diretta.', {flush:true, rate:1.0, pitch:1.02}); }
  return el;
}
...
function confirmScanResult(el,result,silent=false){
  ...
  const aiMeta={brand:result.brand||'',variant:result.variant||'',productType:result.productType||'',packageType:result.packageType||'',estimatedSize:result.estimatedSize||'',isLiquid:!!result.isLiquid,damageNote,expiryDate,confidence:result.confidence||null};
  if(item){
  ...
  rememberLearnedProduct({productName,brand:result.brand||'',variant:result.variant||'',category,unit,productType:result.productType||'',packageType:result.packageType||'',estimatedSize:result.estimatedSize||'',isLiquid:!!result.isLiquid,visibleEvidence:result.visibleEvidence||[],baseName:result.productName||''});
  aiMemory.pendingVerification=false;
  saveAiMemory(); saveAll(); render(); if(!silent){ toast('Articolo aggiornato in casa ✅'); const finalVoice=`Perfetto. Ho aggiornato ${productName} in casa.${expiryDate? ' Scadenza registrata '+expiryDate+'.' : ''}${damageNote && damageNote!=='Integro' ? ' Ho segnato anche lo stato del prodotto.' : ''}`; speakNatural(finalVoice, {flush:true, rate:1.0, pitch:1.02}); } el.classList.remove('voice-active'); const next=document.querySelector('#scannerResults .scan-result:not(.confirmed):not(.bad)'); if(next){ setTimeout(()=>promptScannerConversation(next, next._scanResult||{}, true), 500); } else { resumeLiveAutoScan(1800); if(scannerMicEnabled && liveScanSpeechEnabled){ setTimeout(()=>speakNatural('Scheda completata. Quando vuoi, mostrami il prossimo prodotto.', {flush:true, rate:1.0, pitch:1.02}), 550); } }
}
