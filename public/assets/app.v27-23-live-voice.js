window.SPESA_PRONTA_VERSION='v27.23-live-voice';
...
let liveScanStream = null, liveScanTimer = null, liveScanBusy = false, liveScanActive = false, liveScanStableCount = 0, liveScanLastHint = '', liveScanLastMetrics = null, liveScanPrevSample = null, liveScanLastSpeechKey='', liveScanLastSpeechAt=0, liveScanSpeechEnabled=true;
...
function stopLiveVisionMode(keepMessage=false){
  liveScanActive=false; liveScanStableCount=0; liveScanLastHint=''; liveScanPrevSample=null; liveScanLastSpeechKey=''; if('speechSynthesis' in window) try{ speechSynthesis.cancel(); }catch{}
...
function resetScannerResults(){ stopLiveVisionMode(true); $('#scannerResults').innerHTML=''; $('#scannerPreview').innerHTML=''; setScannerStatus('Risultati svuotati. Puoi riaprire la diretta AI o scattare nuove foto.'); }
...
function startFridgeMode(){
  $('#scannerStatus').textContent='Modalità frigo attiva: apro la diretta AI. Appoggia il prodotto davanti al frigo e, quando è ben centrato, provo a scattare da solo e a leggere marca, scadenza e quantità.';
...
function renderLiveScanStage(note='Metti il prodotto nel riquadro: quando è ben centrato provo a scattare da solo.'){
  ...
  <div class="live-scan-controls"><button class="primary-btn" id="scannerCaptureNowInlineBtn" type="button">⚡ Scatta adesso</button><button class="outline-btn" id="scannerStopLiveBtn" type="button">Chiudi diretta</button></div>
  <p class="live-scan-note"><strong>Guida vocale attiva.</strong> ${esc(note)}</p>`;
...
async function startLiveVisionMode(mode='smart'){
  ...
  try{
    ...
    setScannerStatus('Diretta AI attiva: mostra un solo prodotto per volta, centrato e con la scadenza visibile se possibile.','Diretta attiva. Mostrami un prodotto per volta, ben centrato.');
    queueLiveScanLoop();
  }catch(err){
    stopLiveVisionMode(true);
...
function updateLiveHud(metrics, ready){
  ...
  let voice='';
  let key='start';
  if(metrics.avg<=45){ hint='Serve più luce'; voice='C è poca luce. Avvicinati a una zona più luminosa.'; key='dark'; }
  else if(metrics.avg>=220){ hint='Troppa luce o riflesso'; voice='C è troppo riflesso. Inclina leggermente il prodotto o allontanalo dalla luce.'; key='bright'; }
  else if(!metrics.readable){ hint='Fammi vedere meglio il prodotto'; voice='Non lo leggo bene. Tienilo più fermo e mostra bene la confezione.'; key='unreadable'; }
  else if(metrics.tooBig){ hint="Allontanalo un po'"; voice='Perfetto, ma è troppo vicino. Allontana leggermente il prodotto.'; key='too_big'; }
  else if(metrics.tooSmall){ hint="Avvicinalo un po'"; voice='Avvicina un po il prodotto, così riesco a leggere meglio.'; key='too_small'; }
  else if(Math.abs(metrics.offsetX)>=0.18){
    if(metrics.offsetX<0){ hint='Spostalo a destra'; voice='Sposta leggermente il prodotto verso destra.'; key='move_right'; }
    else { hint='Spostalo a sinistra'; voice='Sposta leggermente il prodotto verso sinistra.'; key='move_left'; }
  }
  else if(Math.abs(metrics.offsetY)>=0.18){
    if(metrics.offsetY<0){ hint="Abbassalo un po'"; voice='Abbassa un po il prodotto nel riquadro.'; key='move_down'; }
    else { hint="Alzalo un po'"; voice='Alza un po il prodotto nel riquadro.'; key='move_up'; }
  }
  else if(!metrics.stable){ hint='Tienilo fermo'; voice='Ottimo, ora tienilo fermo un istante.'; key='stable'; }
  else if(ready){ hint='Perfetto, resto un attimo e scatto'; voice='Perfetto, così va benissimo. Sto per scattare.'; key='ready'; }
  else { hint='Quasi pronto'; voice='Ci siamo quasi. Ruota leggermente il prodotto se vuoi mostrarmi meglio etichetta e scadenza.'; key='almost'; }
...
async function captureLiveFrame(manual=false){
  ...
  setScannerStatus(manual ? 'Sto analizzando lo scatto manuale...' : 'Prodotto centrato: scatto automatico in corso, sto leggendo marca, scadenza, quantità e stato.', manual ? 'Sto analizzando la foto.' : 'Perfetto. Ho scattato. Ora analizzo il prodotto.', true);
...
async function analyzeGroceryDataUrl(dataUrl,fileName='photo.jpg'){
  ...
  if(!quality.ok){
    setScannerStatus(`Non vedo bene: ${quality.reason}. Rifai la foto più vicino, con luce buona, prodotto centrato e data di scadenza visibile.`, 'Non vedo bene il prodotto. Avvicinalo, illuminalo meglio e riprova.', true);
  ...
  if(result.needsRetake){ setScannerStatus(result.reason || 'Non vedo bene, rifai la foto.', 'La foto non è abbastanza chiara. Riproviamo con il prodotto più fermo e ben visibile.', true); addScannerResult({...result,dataUrl}); document.querySelector('.scanning-preview')?.remove(); return; }
  result.dataUrl=dataUrl; result.quality=quality;
  const summary=presentVisionResults(result,dataUrl);
  if(summary.count>1){
    setScannerStatus(`Ho rilevato ${summary.count} prodotti nella scena. ${summary.autoCount?summary.autoCount+' aggiunti automaticamente, ':''}${summary.manualCount?summary.manualCount+' da confermare manualmente.':'nessuno da confermare.'}`, `Ho rilevato ${summary.count} prodotti. ${summary.autoCount?summary.autoCount+' li ho aggiunti in automatico. ':''}${summary.manualCount? 'Controlla gli altri risultati e conferma quelli corretti.' : 'È tutto a posto.'}`, true);
  }else{
    setScannerStatus((result.needsManual?'Controllo manuale: completa i dati, soprattutto nome, quantità e scadenza.':'Vision AI completata: ho incrociato riconoscimento prodotto, OCR e memoria locale. Controlla e conferma: ogni conferma mi aiuta a diventare più precisa con i tuoi prodotti.'), (result.needsManual ? 'Ho letto qualcosa, ma voglio una tua conferma su nome, quantità o scadenza.' : `Perfetto. Ho riconosciuto ${result.productName||'il prodotto'}. Controlla il risultato e conferma se è corretto.`), true);
  }
  document.querySelector('.scanning-preview')?.remove();
}
...
function confirmScanResult(el,result,silent=false){
  ...
  saveAiMemory(); saveAll(); render(); if(!silent){ toast('Articolo aggiornato in casa ✅'); speakNatural(`Perfetto. Ho aggiornato ${productName} in casa.`, {flush:true, rate:1.0, pitch:1.02}); }
}
