window.SPESA_PRONTA_VERSION='v27.25-live-mic-dialog';
...
function stopLiveVisionMode(keepMessage=false){
  liveScanActive=false; liveScanStableCount=0; liveScanLastHint=''; liveScanPrevSample=null; liveScanLastSpeechKey=''; stopScannerMic(true); if('speechSynthesis' in window) try{ speechSynthesis.cancel(); }catch{}
...
function resetScannerResults(){ stopLiveVisionMode(true); scannerMicCurrentResultId=''; scannerMicStep=''; scannerMicLastPrompt=''; $('#scannerResults').innerHTML=''; $('#scannerPreview').innerHTML=''; setScannerStatus('Risultati svuotati. Puoi riaprire la diretta AI o scattare nuove foto.', '', false); }
...
async function startLiveVisionMode(mode='smart'){
  ...
  try{
    ...
    liveScanActive=true; liveScanStableCount=0; liveScanBusy=false; liveScanPrevSample=null; liveScanLastSpeechKey=''; liveScanLastSpeechAt=0;
...
async function runLiveScanLoop(){
  ...
  if(ready){
    liveScanStableCount++;
    if(liveScanStableCount>=2){
      captureLiveFrame(false);
      return;
    }
  }else liveScanStableCount=0;
  queueLiveScanLoop();
}
...
async function captureLiveFrame(manual=false){
  ...
  liveScanBusy=false; liveScanStableCount=0;
...
function addScannerResult(result){
  ...
  if(!result.needsRetake){ setActiveScannerResult(el); if(scannerMicEnabled && !scannerMicListening) startScannerMic(false); setTimeout(()=>promptScannerConversation(el,result,true), 450); }
  else if(liveScanSpeechEnabled){ speakNatural('La foto è da rifare. Posso aiutarti a riprovare dalla diretta.', {flush:true, rate:1.0, pitch:1.02}); }
  return el;
}
